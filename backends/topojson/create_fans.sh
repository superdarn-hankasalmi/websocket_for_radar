...
for rad in "${radars[@]}"
do
	python3 create_geojson_fans.py ${rad} $(eval "echo \$${rad}_beams") $(eval "echo \$${rad}_gates")
done

for rad in "${radars[@]}"
do
	geo2topo -q 1e4 topojsondata=${rad}FANgeojson_45km.json > ${rad}FANStopojson45km.json 
done
